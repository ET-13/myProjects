    ...
    if len(lmList) != 0:
        ...
        if lmList[8][2] < lmList[6][2]: #if point 8 [index tip] (relative to point 2) is lower than positional point 6 (relative to 2), we infer index finger is not out
            ...
            if htm.counter == 1: #if instead of while worked better
                ...
                pyautogui.moveTo(intfingertiptrackercoord1, intfingertiptrackercoord2, duration=.01, tween=pyautogui.easeInOutQuad)   #tracks every 10th of a second, tween function eases it so it wont literally track every single coord input but every other and so on
